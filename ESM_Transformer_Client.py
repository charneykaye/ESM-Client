...
# Divide fasta sequences into groups of 5 and write each batch into a .fasta file
batch_size = 5
for i in range(0, len(fasta_sequences), batch_size):
    batch = fasta_sequences[i:i+batch_size]
    batch_names = sequence_names[i:i+batch_size]
    with open(f'batches/batch_{i//batch_size + 1}.fasta', 'w') as batch_file:
...
#     command = f'docker run --rm --gpus all -v ./example/input:/input -v ./example/output:/output rexpository/esmfold -i {sequence} -o output > ./example/logs/pred.log 2>./example/logs/pred.err '
#     try:
